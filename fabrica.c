...
int main(int argc, char *argv[]) {
    ...
    if (pid == 0) {
        ...
        dup2(pipefd[0], STDIN_FILENO); // Redireccionar entrada estándar
        execl("./productor", "productor", SHM_NAME, SEM_PROD, SEM_CONS, NULL);
        perror("execl");
        exit(1);
    }
    ...
    write(pipefd[1], &N, sizeof(int)); // Enviar N al productor
    close(pipefd[1]); // Cerrar escritura

    // Crear FIFOs para los robots
